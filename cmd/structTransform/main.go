...
var (
  ...
  transformValue = flag.String("t", "", "transform Struct  Demo1->Demo2 ")
)
//go:generate structTransform -t Demo1->Demo2
...
func main() {
  ...
  trObjs := strings.Split(*transformValue, "->")
  if len(trObjs) != 2 || trObjs[1] == "" {
    fmt.Println("error -t")
    os.Exit(2)
  }
  g := &Generator{}
  if sASplit := strings.Split(trObjs[0], "."); len(sASplit) > 1 {
    g.sAPkgName = sASplit[len(sASplit) - 2]
    g.sAName = sASplit[0]
  } else {
    g.sAName = sASplit[0]
  }

  if sBSplit := strings.Split(trObjs[0], "."); len(sBSplit) > 1 {
    g.sBPkgName = sBSplit[len(sBSplit) - 2]
    g.sBName = sBSplit[0]
  } else {
    g.sBName = sBSplit[0]
  }
  ...
  g.sAType = currentStructGetStructType(g, g.sAName)
  g.sBType = currentStructGetStructType(g, g.sBName)
...
type Generator struct {
  ...
  sAName    string
  sAPkgName string
  sAType    *ast.StructType

  sBName    string
  sBPkgName string
  sBType    *ast.StructType
}

func (g *Generator) GetAStruct() {

}

func (g *Generator) GetBStruct() {

}

func currentStructGetStructType(g *Generator, cname string) *ast.StructType {
  for _, v := range g.pkg.files {
    for _, decl := range v.AstFile.Decls {
      ...
      if genDecl.Tok != token.TYPE {
        ...
        //    typeSpec, ok := spec.(*ast.ImportSpec)
        //    fmt.Println(typeSpec, ok, typeSpec.Path.Value, typeSpec.Path.Value[1:len(typeSpec.Path.Value) - 1])
        //    p, err := build.Default.Import(typeSpec.Path.Value[1:len(typeSpec.Path.Value) - 1], "", 0)
        //    fmt.Println(p, err)
        //  }
        //}
      ...
      for _, spec := range genDecl.Specs {
        ...
        return structType
        //fmt.Println(name, typeSpec.Name.Name,structType)
        //os.Exit(0)
...
func (g *Generator) generate() {
  if g.sAType == nil || g.sBType == nil {
    return
  }

  for _, field := range g.sAType.Fields.List {
    ...
    for _, fieldb := range g.sBType.Fields.List {
      fieldTypeb := types.ExprString(fieldb.Type)
      if fieldName == fieldb.Names[0].Name && fieldType == fieldTypeb {
        fmt.Printf("%s:%s.%s,", fieldName, g.sBName, fieldName)
      }
    }
